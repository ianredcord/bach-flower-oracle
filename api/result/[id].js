...
function getRemedyById(id) {
  return remedies.find(r => r.id === id);
}

module.exports = (req, res) => {
  const { id } = req.query;
  const remedyId = parseInt(id, 10);
...
<head>
  ...
  <script>
    ...
    var isCrawler = /facebookexternalhit|twitterbot|linkedinbot|whatsapp|telegrambot|slackbot|discordbot/i.test(userAgent);
    if (!isCrawler) {
